...
import careersHeroBg from "../assets/heroes/careers-growth-journey.webp";
import {
  Briefcase,
  Sparkles,
  MapPin,
...
export const CareersPage: React.FC<CareersPageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="pt-24 pb-20 bg-[#FAF9F5] min-h-screen text-[#111111]" id="careers-page">
      {/* Hero Header */}
      <section className="relative overflow-hidden py-16 sm:py-20 text-center border-b border-[#E8E1D0] bg-white mb-12">
        {/* Background Image with Ambient Glow & Clean Reading Overlay */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
          <motion.div
            initial={prefersReducedMotion ? false : { scale: 1.02, x: "-0.6%" }}
            animate={prefersReducedMotion ? undefined : { scale: [1.02, 1.08, 1.02], x: ["-0.6%", "0.6%", "-0.6%"] }}
          ...
          >
            <img
              ...
              decoding="sync"
              src={careersHeroBg}
              alt="Career growth journey with a modern team and rising staircase"
              className="w-full h-full object-cover object-center opacity-90"
              referrerPolicy="no-referrer"
            />

            {/* Animated details aligned with the objects in the career artwork */}
            <div className="absolute left-[25.6%] top-[75.5%] hidden h-[8.5%] w-[6.4%] overflow-hidden rounded-[2px] md:block">
              <motion.div
                animate={prefersReducedMotion ? undefined : { y: ["0%", "-48%", "0%"] }}
                transition={prefersReducedMotion ? undefined : { duration: 4, ease: "linear", repeat: Infinity }}
                className="space-y-[3px] pt-1 opacity-80"
              >
                {[58, 78, 46, 70, 38, 82, 54, 68].map((width, index) => (
                  <span
                    key={index}
                    className="block h-px bg-[#9A7B16]"
                    style={{ width: `${width}%` }}
                  />
                ))}
              </motion.div>
            ...
            <motion.div
              animate={prefersReducedMotion ? undefined : { scale: [1, 1.12, 1], opacity: [0.55, 0.95, 0.55] }}
              transition={prefersReducedMotion ? undefined : { duration: 2.4, ease: "easeInOut", repeat: Infinity }}
              className="absolute left-[80.5%] top-[68%] hidden h-[10%] w-[7%] rounded-full border border-[#D4AF37]/70 md:block"
            />
            <motion.div
              animate={prefersReducedMotion ? undefined : { rotate: [0, 8, -8, 0], y: [0, -3, 0] }}
              transition={prefersReducedMotion ? undefined : { duration: 3.2, ease: "easeInOut", repeat: Infinity }}
              className="absolute left-[35.2%] top-[78.3%] hidden h-[7%] w-[4%] origin-bottom md:block"
            >
              <span className="absolute left-1/2 top-0 h-full w-px -translate-x-1/2 bg-[#9A7B16]/70" />
              <span className="absolute left-[28%] top-[8%] h-2 w-2 rounded-full bg-[#D4AF37]/55" />
            </motion.div>
            <motion.div
              animate={prefersReducedMotion ? undefined : { scale: [1, 1.08, 1], filter: ["brightness(1)", "brightness(1.18)", "brightness(1)"] }}
              transition={prefersReducedMotion ? undefined : { duration: 2.8, ease: "easeInOut", repeat: Infinity }}
        ...
        {/* Ambient Glow */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] bg-[#D4AF37]/15 rounded-full blur-[100px] pointer-events-none -z-10" />
